...
/**********************************************/
void andn64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size, x86_saved_state_t *state) {
...
void andn32(M32 src, M32 vsrc, M32 *res, x86_saved_state_t *state) {
...
void bextr64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size, x86_saved_state_t *state) {
    uint64_t FLAGS = saved_state64(state)->isf.rflags;
...
void bextr32(M32 src, M32 vsrc, M32 *res, x86_saved_state_t *state) {
    uint32_t FLAGS = saved_state32(state)->efl;
...
void blsr64(M64 src, M64 *res, uint8_t operand_size, x86_saved_state_t *state) {
    uint64_t FLAGS = saved_state64(state)->isf.rflags;
    int SF;
...
void blsr32(M32 src, M32 *res, x86_saved_state_t *state) {
    uint32_t FLAGS = saved_state32(state)->efl;
    int SF;
...
void blsmsk64(M64 src, M64 *res, uint8_t operand_size, x86_saved_state_t *state) {
    uint64_t FLAGS = saved_state64(state)->isf.rflags;
    int SF;
...
void blsmsk32(M32 src, M32 *res, x86_saved_state_t *state) {
    uint32_t FLAGS = saved_state32(state)->efl;
    int SF;
...
void blsi64(M64 src, M64 *res, uint8_t operand_size, x86_saved_state_t *state) {
    uint64_t FLAGS = saved_state64(state)->isf.rflags;
    int SF;
...
void blsi32(M32 src, M32 *res, x86_saved_state_t *state) {
    uint32_t FLAGS = saved_state32(state)->efl;
    int SF;
...
/**********************************************/
void bzhi64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size, x86_saved_state_t *state) {
    uint64_t FLAGS = saved_state64(state)->isf.rflags;
...
void bzhi32(M32 src, M32 vsrc, M32 *res, x86_saved_state_t *state) {
    uint32_t FLAGS = saved_state32(state)->efl;
...
void mulx64(M64 src, M64 *res, M64 *dres, x86_saved_state_t *state, uint8_t operand_size) {
...
void mulx32(M32 src, M32 *res, M32 *dres, x86_saved_state_t *state) {
...
void pdep64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size) {
    int i = 0;
    int k = 0;
...
void pdep32(M32 src, M32 vsrc, M32 *res) {
    int i = 0;
    int k = 0;
...
void pext64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size) {
    int i = 0;
    int k = 0;
...
void pext32(M32 src, M32 vsrc, M32 *res) {
    int i = 0;
    int k = 0;
...
void rorx64(M64 src, M64 *res, uint8_t imm, uint8_t operand_size) {
    if (operand_size == 64) {
        int y = imm & 0x3F;
        uint64_t SRC = src.u64;
...
void rorx32(M32 src, M32 *res, uint8_t imm) {
    int y = imm & 0x1F;
    uint32_t SRC = src.u32;
...
void shlx64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size) {
    if (operand_size == 64) {
...
void shlx32(M32 src, M32 vsrc, M32 *res) {
    int count = vsrc.u8[0] & 31;
    uint32_t temp = src.u32;
    res->u32 = temp << count;
}

void sarx64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size) {
    if (operand_size == 64) {
        int count = vsrc.u8[0] & 63;
...
void sarx32(M32 src, M32 vsrc, M32 *res) {
    int count = vsrc.u8[0] & 31;
    int32_t temp =  src.a32;
    res->a32 = temp >> count;
}

void shrx64(M64 src, M64 vsrc, M64 *res, uint8_t operand_size) {
    if (operand_size == 64) {
        int count = vsrc.u8[0] & 63;
        uint64_t temp = src.u64;
        res->u64 = temp >> count;
...
void shrx32(M32 src, M32 vsrc, M32 *res) {
    int count = vsrc.u8[0] & 31;
    uint32_t temp =  src.u32;
    res->u32 = temp >> count;
}
